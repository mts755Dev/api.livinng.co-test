...
const createAccommodation = async (body) => {
    ...
    const { name, guests ,description, rooms, id, beds, distribution, location, services, price} = body

    if (!name || !body.image || !price || !guests || !body.bathrooms || !description || !rooms || !beds || !distribution || !location || !body.type || !body.size || !body.user || !services) return { error: 'Faltan datos' }
    ...
    if (!existingUser) {
        return { error: `No existe un usuario con id ${body.user} en la base de datos` };
    }
    ...
    if (typeof body.user !== 'string') return { error: `El id del usuario tiene que ser de tipo UUID` }
    ...
    const { type, size, user,image,bathrooms, ...newBody } = body
    newBody.typeId = type
    newBody.sizeId = size
    newBody.userId = user
    newBody.image = image
