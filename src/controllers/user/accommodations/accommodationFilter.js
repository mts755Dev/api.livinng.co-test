...
const accFilter = async (body) => {
    const { type, size, services, guests, bathroom, rooms, beds, price, rating } = body
    ...
    if (guests === null || guests && typeof guests !== 'number') return { error: `guests tiene que ser un numero` }
    if (bathroom === null || bathroom && typeof bathroom !== 'number') return { error: `bathroom tiene que ser un numero` }
    if (rooms === null || rooms && typeof rooms !== 'number') return { error: `rooms tiene que ser un numero` }
    if (beds === null || beds && typeof beds !== 'number') return { error: `beds tiene que ser un numero` }
